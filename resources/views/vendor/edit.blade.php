...
<x-app-layout>
    <x-slot name="header">
        ...
        <div class="ml-2 mt-2 md:mt-0 md:ml-8">
            <button type="button" class="w-32 text-center text-sm text-white bg-indigo-500 border-0 py-1 px-2 focus:outline-none hover:bg-indigo-700 rounded " onclick="location.href='{{ route('vendor_index') }}'" >仕入先一覧</button>
        </div>
        <div class="ml-4 mt-2 md:mt-0 md:ml-8">
            <button type="button" class="w-32 text-center text-sm text-white bg-indigo-500 border-0 py-1 px-2 focus:outline-none hover:bg-indigo-700 rounded " onclick="location.href='{{ route('vendor_show',['id'=>$vendor->id]) }}'" >仕入先詳細</button>
        </div>
        ...
        <form method="post" action="{{ route('vendor_update',['id'=>$vendor->id])}}" >
            @csrf
            @method('PUT')
            <div class="">
                ...
                <div class="flex pl-0 mt-0">
                    ...
                    <input class="pl-2 ml-0 md:ml-2 w-24 h-6 text-sm items-center bg-gray-100 border rounded" name="vendor_id"  value="{{ $vendor->id }}"></input>
                </div>

                <div class="flex pl-0 mt-2 md:mt-2 ">
                    <label for="vendor_name" class="leading-7 text-sm  text-gray-800  ">社名</label>
                    <input class="ml-3 pl-2 w-32 h-6 text-sm items-center bg-gray-100 border rounded" name="vendor_name" value="{{ $vendor->vendor_name }}"></input>
                </div>
                </div>
                ...
                <div class="flex pl-0 mt-2 md:mt-2">
                    ...
                    <textarea class="pl-0 ml-0 text-sm md:ml-4 w-full md:w-1/2  items-center bg-gray-100 border rounded" name="vendor_info" rows="10">{{ $vendor->vendor_info }}</textarea>
                </div>
