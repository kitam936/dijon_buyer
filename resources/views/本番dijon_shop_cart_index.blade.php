<x-app-layout>
    <x-slot name="header">
        ...
        <div class="md:flex">
            ...
            <div class="flex">
                <div class="ml-2 mt-0 md:ml-4 md:mt-0">
                    <button type="button" class="w-32 h-8 text-center text-sm text-white bg-green-500 border-0 py-1 px-2 focus:outline-none hover:greenigo-700 rounded " onclick="location.href='{{ route('cart_edit') }}'" >カート修正</button>
                </div>
                <div class="ml-4 mt-0 md:ml-4 md:mt-0">
                    <button type="button" class="w-32 h-8 text-center text-sm text-white bg-green-500 border-0 py-1 px-2 focus:outline-none hover:bg-green-700 rounded " onclick="location.href='{{ route('cart_create') }}'" >オーダーを続ける</button>
                </div>
            </div>
        </div>
        ...
        <div class="px-2 w-full md:ml-10 mt-4 mb-4 flex ">
            <button type="submit" class="w-32 h-8 text-center text-sm text-white bg-pink-500 border-0 py-1 px-2 focus:outline-none hover:bg-pink-700 rounded ">発注確定</button>
        </div>
        ...
        <div class="flex">
                <div class="flex">
                    <div class="md:pl-2 mt-0">
                        ...
                        <div disable class="pl-2 w-28 h-6 text-sm items-center bg-gray-100 border rounded" name="total_pcs" value=""> {{ $cart_total->total_kingaku ?? 0}} 円</div>
                    </div>
                </div>
                <div class="md:ml-0 ml-2 flex">
                    <div class="md:pl-2 mt-0">
                        <label for="total_pcs" class="leading-7 text-sm  text-gray-800 ">原価合計</label>
                        <div disable class="pl-2 w-28 h-6 text-sm items-center bg-gray-100 border rounded" name="total_pcs" value=""> {{ $cart_total->total_gedai ?? 0}} 円</div>
                    </div>
                </div>
                 <div class="pl-2 mt-0">
                    <label for="total_pcs" class="leading-7 text-sm  text-gray-800 ">合計数</label>
                    <div disable class="pl-2 w-20 h-6 text-sm items-center bg-gray-100 border rounded" name="total_pcs" value=""> {{ $cart_total->total_pcs ?? 0}} 枚</div>
                </div>
            </div>
        </form>
    ...
    <div class="py-0 border md:pl-24 md:ml-20 md:w-1/2">
        {{-- <h1>店舗Report</h1> --}}
                        ...
                        <tr>
                            ...
                            <td class="w-1/15 md:px-4 py-1">
                                <form method="POST" action="{{ route('cart_add') }}">
                                    ...
                                    <div disable class="w-12 ml-2 text-center rounded bg-gray-100" type="text" name="pcs" value="{{ $cart->pcs }}">{{ $cart->pcs }}</div>
