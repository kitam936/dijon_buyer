...
<x-app-layout>
    <x-slot name="header">
            ...
            <div class="lg:flex">
                <div class="md:flex">
                    ...
                    @foreach ($brands as $brand)
                        <option value="{{ $brand->brand_id }}" @if(\Request::get('brand_code') == $brand->brand_id ) selected @endif >{{ $brand->brand_id  }}={{ $brand->brand_name  }}</option>
                    @endforeach
                    </select>
                </div>
                <div class="flex">
                ...
                <div>
                    <button type="button" class="w-20 h-8 bg-blue-300 text-white ml-2 hover:bg-indigo-600 rounded " onclick="location.href='{{ route('data.predata_index') }}'" class="mb-2 ml-2 text-right text-black bg-indigo-300 border-0 py-0 px-2 focus:outline-none hover:bg-indigo-300 rounded ">全表示</button>
                </div>
                </div>
            </div>
        </form>
        ...
        <form method="POST" action="{{ route('data.predata_upload') }}" class=" p-1" enctype="multipart/form-data">
            @csrf
            {{-- <label for="predata" class="leading-7 text-ml text-gray-600">csv取込</label> --}}
            <input type="file" id="predata" name="predata" accept=“.csv” class="w-2/3 md:w-1/3 ml-0 bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
            <button type="submit" class="w-24 h-9 text-sm text-white bg-green-500 border-0 py-1 px-4 focus:outline-none hover:bg-green-600 rounded">csv更新</button>
        </form>
            ...
            <table class="md:w-2/3 bg-white table-auto w-full text-center whitespace-no-wrap">
               <thead>
                    <tr>
                        ...
                        <th class="w-1/15 md:px-4 py-1 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100">status</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        @if( $product->done )
                        <td class="w-4/15 md:px-4 text-green-600 py-1 text-center">登録済</td>
                        @else
                        <td class="w-4/15 md:px-4 py-1 text-left"></td>
                        @endif
                        @if(Auth::user()->role_id <= 3)
                        ...
                        @endif
                        {{-- <td class="w-1/15 md:px-4 py-1 text-right">{{ number_format($product->local_price )}}</td> --}}
